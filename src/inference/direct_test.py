...
def main():
    ...
    # max_new_tokens = 2048 # Value from config if generate.py was used
    temperature = 0.4
    top_k = 40
    top_p = 0.9
    ...
    # logger.info(f"Generating with max_new_tokens: {max_new_tokens}") # This was for max_new_tokens logic
    logger.info(f"Using max_length for model.generate(): {MAX_SEQ_LENGTH}")
    logger.info(f"Generation params: temp={temperature}, top_k={top_k}, top_p={top_p}, rep_penalty={repetition_penalty}, min_p={min_p}")
    logger.info(f"Calling model.generate() with input_ids_length: {input_ids_length}, model max_seq_length: {MAX_SEQ_LENGTH}")
    ...
    with torch.no_grad():
        generated_ids = model.generate(
            **model_inputs,
            ...
            min_p=min_p,
            max_length=MAX_SEQ_LENGTH, # Use max_length 
    ...
    logger.info(f"Generated token IDs (last 50 of new tokens): {generated_ids[0, input_ids_length:][-50:].tolist() if generated_ids.shape[1] > input_ids_length else 'No new tokens'}")
    ...
    try:
        ...
        sf.write(OUTPUT_FILE, audio_output.numpy(), samplerate=dac_interface.sr) # Convert to numpy for soundfile
